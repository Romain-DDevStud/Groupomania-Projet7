...
exports.newUSer = (req, res, next) => {
    ...
    if (error) {
        res.status(442).json({ error: "Données saisies invalides!"});
    } else {
        next();
    }
};
...
exports.postContent = (req, res, next) => {
    // SI le contenu est défini : validation du contenu
    if (req.body.content) {
    ...
    // SI ni la photo, ni le contenu ne sont définis : requête rejettée
    } else if (!req.body.content && !req.file) {
    res.status(422).json({ error: "Envoyer au moins une image ou un texte !" });

    // SI le contenu n'est pas défini, et l'image est définie : validation déjà effectuée par multer!
    } else {
    next();
    }
};
