...
exports.logout = (req, res, next) => {
    // on remplace le cookie en cours par un cookie vide
...
/* Réponse à la vérification de la bonne authentification de l'utilisateur (arrive après le middleware "auth" */
...
/* Modifier la photo de profil utilisateur */
...
exports.changeAdmin = (req, res, next) => {
    ...
    connection.execute(sql, sqlParams, (error, results, fields) => {
        if (error) {
            ...
            res.status(201).json({ message: 'Droits d\'administrateur modifiés' });
        }
    });
    connection.end();
}
